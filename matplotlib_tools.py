...
class Ruler(AxesWidget):
    ...
    def __init__(self,
        ...
        used_textprops = dict(
                        xy=(0.5, 0),                 # centered horizontally
                        xycoords='figure fraction',  # relative to whole figure
                        xytext=(100, 100),             # 100px up
        ...
        if textprops is not None:
            used_textprops.update(textprops)
    ...
    def _move_ruler(self, event):
        ...
        if self._end_b_lock is True:
            ...
            self._ruler.set_data(pos_a, pos_b)
            ax_, ay_ = marker_a_coords
            cx_, cy_ = marker_c_coords
            self._marker_a.set_data([ax_], [ay_])
            self._marker_b.set_data([event.xdata], [event.ydata])
            self._marker_c.set_data([cx_], [cy_])
    ...
    def _set_midline_marker(self):
        ...
        mx, my = self.midline_coords
        self._marker_b.set_data([mx], [my])
